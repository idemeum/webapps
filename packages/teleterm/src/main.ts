...
import { generateGrpcCertificates } from './mainProcess/generateGrpcCertificates';
...
async function startMain(): Promise<void> {
  ...
  await generateGrpcCertificates(settings.certsDir);
  // init main process
  ...
  app.on('certificate-error', (event, webContents, url, error, certificate, callback) => {
    if (settings.dev && url.includes('://localhost:8080/') && error === 'net::ERR_CERT_AUTHORITY_INVALID') {
