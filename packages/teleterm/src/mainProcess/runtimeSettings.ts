...
async function getRuntimeSettings(): Promise<RuntimeSettings> {
  ...
  const { tshAddress, sharedAddress } = await getGprcAdresses();
  ...
  const tshd = {
    ...
    flags: ['daemon', 'start', `--addr=${tshAddress.startsWith('unix') ? tshAddress : 'tcp://' + tshAddress}`, `--certsDir=${getCertsDir()}`],
  };
  const sharedProcess = {
...
function getTshBinaryPath() {
  if (app.isPackaged) {
    return path.join(getBinDir(), process.platform === 'win32' ? 'tsh.exe': 'tsh');
  }
...
async function getGprcAdresses(): Promise<{ tshAddress: string, sharedAddress: string }> {
  switch (process.platform) {
    case 'win32': {
      const [tshPort, sharedPort] = await Promise.all([getAvailablePort(), getAvailablePort()]);
      return {
