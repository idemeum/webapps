...
import { readGrpcCertificates } from 'teleterm/mainProcess/readGrpcCertificates';
...
async function initializeServer(runtimeSettings: RuntimeSettings): Promise<void> {
  ...
  const {caCert, serverCert, serverKey} = await readGrpcCertificates(runtimeSettings.certsDir);

  server.bindAsync(address, ServerCredentials.createSsl(caCert, [{
